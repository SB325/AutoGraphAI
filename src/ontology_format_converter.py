...
def extension_test(filename: str, expected_ext: str | Sequence[str], verbose: bool = False) -> bool:
    ...
    if ext in allowed:
        if verbose: print(f"Extension is valid {ext}") 
        return True
    else:
        if verbose: print(f"Wrong extension: {ext} was provided instead of {expected_ext}.")
        return False
...
def rdf_xml_to_ttl(inputfile: str, outputfile: str, verbose: bool):
    try:
        if verbose:print(f"Parsing {inputfile} to {outputfile}") 

        if not extension_test(inputfile, ['.rdf','.xml']):
            if verbose: print(f"Inputfile is not \'.rdf\' or \'.xml\'.") 
        else:
            g.parse(inputfile, format="xml")
            if verbose: print(f"Parsed {inputfile} to {outputfile}") 

            if verbose: print(f"Serializing {inputfile} to {outputfile}") 
            if extension_test(outputfile, '.ttl'):
                g.serialize(destination=outputfile, format="turtle")
            else:
                if verbose: print(f"Outputfile is not '.ttl'.")
            if verbose: print(f"Written to {outputfile}")
...
def ttl_to_rdf_xml(inputfile: str, outputfile: str, verbose: bool):
    try:
        ...
        if not extension_test(inputfile, '.ttl'):
            if verbose: print(f"Inputfile is not '.ttl'.")
        else:
            ...
            if extension_test(outputfile, ['.rdf','.xml']):
                ...
                if verbose: print(f"Outputfile is not \'.rdf\' or \'.xml\'.")
            if verbose: print(f"Written to {outputfile}")
    ...
    try:
        ...
        if not extension_test(inputfile, '.ttl'):
            ...
            if extension_test(outputfile, '.jsonld'):
                ...
                if verbose: print(f"Outputfile is not '.jsonld'.")
            if verbose: print(f"Written to {outputfile}") 
    except Exception as e:
        print(f"Error parsing and serializing  {inputfile} to {outputfile}:\n{e}")
